...
class StippleFrameResult(val cols: Int, val rows: Int) {
    ...
    /** Stable per-cell jitter, as a fraction of cell size, so dots don't sit on a rigid grid. */
 ...
 * grid cell gets a single dot whose radius scales with local tone and whose
 * position gets a small *stable* per-cell jitter (from [hashNoiseF] with a
 * fixed `t`, so it doesn't swim frame to frame) — cheap enough for 30fps,
 * and visually close to the weighted look without the relaxation.
 */
object StipplePipeline {
    ...
    fun process(
        ...
        for (i in 0 until n) {
            ...
            val x = i % cols
            val y = i / cols
            // Fixed t=0: a stable per-cell hash, not a time-varying one, so
            // dot positions don't swim frame to frame on a live feed.
            result.offsetXFraction[i] = (hashNoiseF(x, y, 0f, 11f) - 0.5f) * 0.6f
            result.offsetYFraction[i] = (hashNoiseF(x, y, 0f, 53f) - 0.5f) * 0.6f
